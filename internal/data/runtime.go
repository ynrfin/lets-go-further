...
// Implement a MarshalJSON() methon on the Runtime type so that it satisfies the
// json.Marshaller interfce. This should return the JSON-encoded value for the movie
// runtime (in our case, it will return a string in the format "<runtime> mins")
func (r Runtime) MarshalJSON() ([]byte, error) {
	// Generate a string containing the movie runtime in the required format.
	jsonValue := fmt.Sprintf("%d mins", r)

	// user strconv.Quote() function on the string to wrap it in double quotes. It
...
// json.Unmarshaller interface. IMPORTANT: Because UnmarshalJSON() needs to modify the
// receiver (our Runtime type), we must use a pointer receiver for this to work
// correctly. Otherwise, we wil only be modifying a copy (which is then discarded when
