...
// Define custom Logger type. This holds the output destination that the log entries
// will be written to, the minimum severity level that log entries will be written for,
// plus a mutx for coordinating writes.
type Logger struct {
	out      io.Writer
	minLevel Level
	mu       sync.Mutex
...
func (l *Logger) PrintErr(err error, properties map[string]string) {
...
func (l *Logger) print(level Level, message string, properties map[string]string) (int, error) {
	...
	// ogger, then return with no further action
	if level < l.minLevel {
		return 0, nil
	}
	...
	// plain-text or error message instead
